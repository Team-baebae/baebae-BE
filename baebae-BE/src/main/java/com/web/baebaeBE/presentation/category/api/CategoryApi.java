...
public interface CategoryApi {
    ...
    @Operation(summary = "카테고리 이름 수정", description = "카테고리 ID와 새로운 카테고리 이름을 받아 카테고리 이름을 수정합니다.")
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "수정 성공",
                    content = @Content(mediaType = "application/json",
                            schema = @Schema(implementation = CategoryResponse.CategoryInformationResponse.class)))
    })
    ResponseEntity<CategoryResponse.CategoryInformationResponse> updateCategoryName(
            @Parameter(description = "카테고리의 ID", required = true) @PathVariable Long categoryId,
            @Parameter(description = "수정할 카테고리 이름 정보", required = true) @RequestBody CategoryRequest.UpdateCategoryName updateCategoryName
    );
