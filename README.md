...
### __```Spring Security```__
  ...
  • 애플리케이션의 보안을 강화하고, 사용자 인증 및 권한 관리를 효율적으로 수행

### __```JPA```__
  • Java ORM 기술로 데이터베이스와 객체 지향 프로그래밍 사이의 "임피던스 불일치"를 해결
  • 데이터베이스 테이블과 Java 객체 간의 매핑
  • 데이터베이스 작업을 간소화하고 데이터 관리를 더욱 효율적으로 수행

### __```QueryDSL```__
  • 타입 안전성을 보장하는 SQL과 유사한 문법을 제공하여 복잡한 쿼리를 쉽게 작성할 수 있게 해주는 프레임워크
